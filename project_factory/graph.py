...
def write_tests(state: S) -> dict:
    ...
    claude(
        ...
        "[role=\"alert\"]/[aria-live] query also matches the toast container "
        "and the framework's route announcer — a strict-mode violation on 3 "
        ...
        "  3. NEVER anchor a wait on a bare number or a value the page chrome "
        "may also contain. Waiting for '100' on container C-1001 is satisfied "
        "instantly by a breadcrumb rendering 'C 1001'. Anchor on a token "
        "unique to the thing you are waiting for (a SKU, a row test id).\n"
